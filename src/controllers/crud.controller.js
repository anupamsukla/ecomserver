...
const getAllprocducts = (model) => async (req, res) => {
  try {
...
const getAllPaginated = (model) => async (req, res) => {
  try {
    ...
    const item = await model
      .find({ $and: [{ $or: isGender }, { $or: category }] })
      .sort(isSort)
      .skip(skilValues)
      .limit(size)
      .lean()
      .exec()
    const totalProducts = await model
      .find({ $and: [{ $or: isGender }, { $or: category }] })
      .countDocuments()

    const totalPages = Math.ceil(totalProducts / size)

    return res.status(200).send({ item, totalPages, totalProducts })
  } catch (e) {
    return res.status(500).json({ message: e.message, status: 'Failed' })
  }
}
...
const updateCoupon = (model) => async (req, res) => {
  try {
    const coupon = await model
      .findOne({ couponCode: req.body.couponCode })
      .lean()
      .exec()
    ...
    if (!coupon) {
      return res
        .status(400)
        .json({ status: 'Failed', message: 'Invalid Coupon Code' })
    }
    ...
    if (!!user.length) {
      return res
        .status(400)
        .json({ status: 'Failed', message: 'Already used this coupon' })
    }

    const item = await model
      .findByIdAndUpdate(
        coupon._id,
        { user: [...coupon.user, req.user._id] },
        { new: true },
      )
      .lean()
      .exec()
    ...
    return res
      .status(201)
      .json({ couponCode: item.couponCode, discountValue: item.discountValue })
  } catch (e) {
    return res.status(500).json({ message: e.message, status: 'Failed' })
  }
}
...
const removeCoupon = (model) => async (req, res) => {
  try {
    ...
    const item = await model
      .findByIdAndUpdate(
        coupon._id,
        { user: coupon.user.filter((e) => e != req.user._id) },
        { new: true },
      )
      .lean()
      .exec()
